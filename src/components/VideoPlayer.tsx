...
import { Heart, FileText, Folder } from "lucide-react";
...
const VideoPlayer = ({ candidate, isActive, onSwipeRight, onPortfolioClick }: VideoPlayerProps) => {
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      if (e.code === 'Space' && isActive && videoRef.current) {
        e.preventDefault();
        ...
        <AspectRatio ratio={9 / 16}>
          ...
          <div className="absolute bottom-24 right-6 hidden md:flex flex-col gap-4">
            ...
            <button
              ...
              className="h-14 w-14 flex items-center justify-center rounded-full bg-accent text-accent-foreground shadow-[0_0_30px_hsl(280_100%_60%/0.5)] transition-all duration-300 hover:scale-110 hover:shadow-[0_0_40px_hsl(280_100%_60%/0.7)] group"
            >
              <Folder className="h-6 w-6 group-hover:animate-pulse" />
              <div className="absolute inset-0 rounded-full border-2 border-accent/50 animate-ping-slow" />
            </button>
            
            {/* Resume Button */}
            <button
              onClick={onSwipeRight}
              className="h-14 w-14 flex items-center justify-center rounded-full bg-primary text-primary-foreground shadow-[0_0_30px_hsl(160_100%_45%/0.5)] transition-all duration-300 hover:scale-110 hover:shadow-[0_0_40px_hsl(160_100%_45%/0.7)] group"
            >
